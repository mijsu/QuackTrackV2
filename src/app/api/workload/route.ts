...
export async function GET(request: NextRequest) {
  try {
    ...
    const workloadData = faculty.map(f => {
      ...
      // ── Merge-aware unit counting ──
      // Group schedules by (day, subjectId, sectionId) so that consecutive
      // time slots for the same task group are counted only once.
      // For lecture_and_lab subjects, count only once across all groups.
      const taskGroups = new Map<string, typeof schedules[0][]>()
      ...
      for (const [, group] of taskGroups) {
        ...
        if (subj) {
          if (subj.subjectType === 'lecture_and_lab') {
            if (!countedSplit.has(subj.id)) {
              totalUnits += subj.units
              countedSplit.add(subj.id)
            }
          } else {
            totalUnits += subj.units
          }
