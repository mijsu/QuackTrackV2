...
export async function GET(
  ...
  try {
    ...
    for (const [fid, schedules] of facultySchedules) {
      ...
      // Match the algorithm's simple counting: each unique (day, subjectId, sectionId)
      // task group counts units once (no merge dedup — the algorithm counts every task).
      // For lecture_and_lab subjects, only count subject.units once across all components
      // since the algorithm splits them (e.g., 3+1 for a 4-unit subject).
      const countedTaskGroups = new Set<string>()
      const countedSplitSubjects = new Set<string>()

      for (const [, group] of taskGroups) {
        // For multi-slot tasks, only count once per task group
        const firstSlot = group[0]
        const taskKey = `${firstSlot.day}|${firstSlot.subjectId}|${firstSlot.sectionId}`
        if (!countedTaskGroups.has(taskKey)) {
          const subj = firstSlot.subject
          if (subj.subjectType === 'lecture_and_lab') {
            if (!countedSplitSubjects.has(subj.id)) {
              totalUnits += subj.units
              countedSplitSubjects.add(subj.id)
            }
          } else {
            totalUnits += subj.units
          }
