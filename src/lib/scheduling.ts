...
function scoreSlot(
  ...
  // Preferred day bonus — HIGH priority: faculty preferences must be respected
  // to prevent overloading. A faculty who prefers a day should strongly win over
  // one who doesn't, ensuring balanced load distribution across willing faculty.
  if (isPrefDay) score += 40
  else score -= 15

  // Preferred time bonus
  if (isPrefTime) score += 25
  else score -= 10

  // ── Section day distribution (0-30 points) ──
  ...
  else if (sectionCount >= 2) score -= 8
  else if (sectionCount === 0) score += 12
  // Bonus for spreading to empty days
  else score += 5

  // ── Faculty day load balancing (0-25 points) ──
  ...
  if (facultyDayLoad >= 3) score -= 15
  else if (facultyDayLoad >= 2) score -= 5
  else if (facultyDayLoad === 0) score += 10
  else score += 3

  // ── Faculty total load balancing (0-45 points) ──
  // Strongly reward under-utilized faculty and heavily penalize nearly-full ones
  // to ensure even distribution of units across all eligible faculty and prevent
  // overloading any single faculty member.
  if (effectiveMaxUnits > 0) {
    ...
    if (utilization <= 0.50) score += 40       // < 50% → strongly favored
    else if (utilization <= 0.70) score += 25   // 50-70% → moderately favored
    else if (utilization <= 0.85) score += 10   // 70-85% → slightly favored
    else if (utilization <= 1.00) score -= 25   // 85-100% → penalized (nearing cap)
    else score -= 60                             // > 100% → heavily penalized (over cap)
  }
  ...
  if (globalLoad <= avgLoad) score += 12
  else score -= 8
...
export async function generateSchedules(input: GenerationInput): Promise<GenerationResult> {
  const { semester, academicYear, departmentIds, configId } = input

  const deptIds = departmentIds && departmentIds.length > 0 ? departmentIds : undefined
  console.log(`🔧 Schedule generation started: semester=${semester}, academicYear=${academicYear}, departments=${deptIds?.join(',') || 'all'}`)
  ...
  try {
    ...
    // Departments are the primary scheduling entities. When one or more departments
    // are selected, we scope all data to those departments. The classType of selected
    // departments determines faculty eligibility:
    //   - If ALL selected departments are 'executive' → only masteral faculty
    //   - If ANY selected department is 'regular' → all faculty types eligible
    //   - If NO departments selected ("All Departments") → include everything
    let targetDeptIds: string[] | undefined = deptIds
    let facultyTypeFilter: string | null = null  // null = no filter (all faculty types)
    ...
    if (deptIds && deptIds.length > 0) {
      ...
      // Determine faculty type filter: if ALL selected departments are executive,
      // only masteral faculty are eligible; otherwise all faculty types can teach.
      const allExecutive = departments.every(d => d.classType === 'executive')
      const allRegular = departments.every(d => d.classType === 'regular')
      if (allExecutive) {
        facultyTypeFilter = 'masteral'
      }
      // For mixed types or all-regular, no facultyType filter is applied.

      console.log(`🏛️ Using ${departments.length} department(s): ${departments.map(d => `${d.name}(${d.classType})`).join(', ')}`)
      if (allExecutive) console.log('   → Executive only: masteral faculty required')
      else if (allRegular) console.log('   → Regular only: all faculty types eligible')
      else console.log('   → Mixed types: all faculty types eligible')
    } else {
      console.log(`🏛️ No departments specified — including all departments`)
    }
    ...
    if (facultyTypeFilter) facultyWhere.facultyType = facultyTypeFilter
    ...
    // When departments are selected, all their subjects and sections are included
    // automatically (scoped by departmentId in the queries above).
    // No additional classType filtering is needed.
    const filteredSubjects = subjects
    const filteredSections = sections

    const deptLabel = targetDeptIds
      ? `${targetDeptIds.length} department(s)${facultyTypeFilter ? ` (${facultyTypeFilter})` : ''}`
      : 'all departments'

    console.log(`📊 Data found: ${filteredSubjects.length} subjects, ${faculty.length} faculty, ${filteredSections.length} sections (${deptLabel})`)
    ...
    if (filteredSubjects.length === 0 || faculty.length === 0 || sections.length === 0) {
      ...
      if (filteredSubjects.length === 0) missingItems.push({
        label: `Subjects (semester="${semester}"${targetDeptIds ? `, departments="${deptIds?.join(',')}"` : ''})`,
        found: filteredSubjects.length,
        totalInSemester: allSubjectsInSemester.length,
        needed: 'At least 1 subject in the selected departments matching the semester',
      })
      if (faculty.length === 0) missingItems.push({
        label: `Faculty${targetDeptIds ? ` (departments="${deptIds?.join(',')}")` : ''}`,
        found: 0,
        totalInSemester: 0,
        needed: 'At least 1 eligible faculty member in the selected departments',
      })
      if (sections.length === 0) missingItems.push({
        label: `Sections (semester="${semester}"${targetDeptIds ? `, departments="${deptIds?.join(',')}"` : ''})`,
        found: filteredSections.length,
        totalInSemester: allSectionsInSemester.length,
        needed: 'At least 1 section in the selected departments matching the semester',
      })
      ...
      return {
        ...
        conflictCount: 0,
        status: 'failed',
        message: `Cannot generate schedule — missing required data for the selected semester and departments.`,
        missingItems,
      }
    }
    ...
    for (const task of shuffled) {
      ...
      const eligibleFaculty = faculty.filter(f => {
        // STRICT: Faculty can ONLY teach subjects from their own department.
        // Cross-department assignments are not allowed — even with matching
        // specialization — to ensure each department's faculty handles its own
        // curriculum and load stays within the department.
        if (f.departmentId !== task.subjectDepartmentId) return false

        const hasMatchingSpec = specializationMatches(f.specialization, task.subjectRequiredSpec)
        // If the subject requires a specific specialization, the faculty must match it.
        // Faculty type filtering (e.g., masteral-only for executive departments) is
        // handled at the query level above — no additional check needed here.
        if (task.subjectRequiredSpec && !hasMatchingSpec) return false
      ...
      // Simple per-task unit tracking (used for real-time cap enforcement during loop).
      // After generation completes, we recompute from generatedSchedules for accuracy.
    ...
    // ── RECOMPUTE facultyUnits from generatedSchedules ──
    // This ensures tracked values match the database. We query subject units
    // to match the faculty-load API's counting: sum of subject.units per
    // task group, with lecture_and_lab dedup (count once per subject).
    const subjectUnits = new Map<string, { units: number; subjectType: string }>()
    try {
      const subjects = await db.subject.findMany({
        where: { id: { in: [...new Set(generatedSchedules.map(s => s.subjectId))] } },
        select: { id: true, units: true, subjectType: true },
      })
      for (const subj of subjects) {
        subjectUnits.set(subj.id, { units: subj.units, subjectType: subj.subjectType })
      }
    } catch { /* ignore db errors */ }

    const schedByFaculty = new Map<string, typeof generatedSchedules[0][]>()
    for (const s of generatedSchedules) {
      const arr = schedByFaculty.get(s.facultyId) || []
      arr.push(s)
      schedByFaculty.set(s.facultyId, arr)
    }

    for (const [fid, scheds] of schedByFaculty) {
      const taskGroups = new Map<string, typeof scheds[0][]>()
      for (const s of scheds) {
        const tk = `${s.day}|${s.subjectId}|${s.sectionId}`
        const arr = taskGroups.get(tk) || []
        arr.push(s)
        taskGroups.set(tk, arr)
      }
      const countedSplit = new Set<string>()
      let recomputed = 0
      for (const [, group] of taskGroups) {
        const first = group[0]
        const subjInfo = subjectUnits.get(first.subjectId)
        if (subjInfo) {
          if (subjInfo.subjectType === 'lecture_and_lab') {
            if (!countedSplit.has(first.subjectId)) {
              recomputed += subjInfo.units
              countedSplit.add(first.subjectId)
            }
          } else {
            recomputed += subjInfo.units
          }
      ...
      facultyUnits.set(fid, recomputed)
    }

    // ── Load audit & cleanup ──
    // Remove entire task groups from overloaded faculty to enforce the hard cap.
    // Uses composite key matching since schedule IDs aren't available pre-insertion.
    const conditionsToDelete: Array<Record<string, string>> = []
    let trimmedCount = 0
    for (const [fid, scheds] of schedByFaculty) {
      const f = faculty.find(ff => ff.id === fid)
      if (!f) continue
      const contractLimit: Record<string, number> = { casual: 18, part_time: 18, full_time: 21, permanent: 21 }
      const cl = contractLimit[f.contractType ?? 'full_time'] ?? 21
      const mu = getEffectiveMaxUnits(f.maxUnits, (f.preferences || []) as FacultyPreferenceData[], semester)
      const eff = Math.min(mu, cl)
      const tracked = facultyUnits.get(fid) || 0
      if (tracked <= eff) continue

      // Group schedules by task key (day, subjectId, sectionId)
      const taskMap = new Map<string, typeof scheds[0][]>()
      for (const s of scheds) {
      ...
      // Sort tasks by unit value descending (remove biggest subjects first)
      const tasks = [...taskMap.entries()].map(([key, group]) => {
        const first = group[0]
        const info = subjectUnits.get(first.subjectId)
        const units = info?.units ?? 3
        return { key, group, units }
      }).sort((a, b) => b.units - a.units)

      const excess = tracked - eff
      let removed = 0

      for (const task of tasks) {
        if (removed >= excess) break
        // Create delete conditions for each slot in this task group
        for (const s of task.group) {
      ...
      // Recompute remaining units from kept tasks
      const removedKeys = new Set(tasks.filter(t => {
        let r = 0
        for (const task of tasks) {
          if (r >= excess) return false
          if (task.key === t.key) return true
          r += task.units
        }
        return false
      }).map(t => t.key))

      const keptTasks = tasks.filter(t => !removedKeys.has(t.key))
      let remaining = 0
      const splitSeen = new Set<string>()
      for (const task of keptTasks) {
        const info = subjectUnits.get(task.group[0].subjectId)
        if (info) {
          if (info.subjectType === 'lecture_and_lab') {
            if (!splitSeen.has(task.group[0].subjectId)) {
              remaining += info.units
              splitSeen.add(task.group[0].subjectId)
            }
          } else {
            remaining += info.units
          }
      ...
      facultyUnits.set(fid, remaining)
      trimmedCount++
      console.log(`✂️  Removed ${removed} units from ${f.name} (now ${remaining}/${eff})`)
    }

    // Delete trimmed schedule records from database.
    // Must delete associated conflicts first due to FK constraints.
    if (conditionsToDelete.length > 0) {
      // First, fetch the actual schedule IDs that match our composite criteria
      const schedulesToDelete = await db.schedule.findMany({
        where: { scheduleVersionId: version.id, OR: conditionsToDelete },
        select: { id: true },
      })
      const deleteIds = schedulesToDelete.map(s => s.id)

      if (deleteIds.length > 0) {
        // Delete conflicts referencing these schedules (both scheduleId1 and scheduleId2)
        await db.conflict.deleteMany({
          where: {
            OR: [
              { scheduleId1: { in: deleteIds } },
              { scheduleId2: { in: deleteIds } },
            ],
          },
        })
        // Now safe to delete the schedules
        await db.schedule.deleteMany({
          where: { id: { in: deleteIds }, scheduleVersionId: version.id },
        })
      }

      // Remove from generatedSchedules array
      const matchKey = (s: typeof generatedSchedules[0]) =>
        `${s.day}|${s.startTime}|${s.endTime}|${s.subjectId}|${s.facultyId}|${s.sectionId}`
      const deleteKeys = new Set(conditionsToDelete.map(c => `${c.day}|${c.startTime}|${c.endTime}|${c.subjectId}|${c.facultyId}|${c.sectionId}`))
      for (let i = generatedSchedules.length - 1; i >= 0; i--) {
        if (deleteKeys.has(matchKey(generatedSchedules[i]))) {
          generatedSchedules.splice(i, 1)
        }
      }
    }
    ...
    for (const f of faculty) {
      ...
      const mu = getEffectiveMaxUnits(f.maxUnits, (f.preferences || []) as FacultyPreferenceData[], semester)
      const eff = Math.min(mu, cl)
      const tracked = facultyUnits.get(f.id) || 0
      if (tracked > maxTracking) maxTracking = tracked
      if (eff > maxEff) maxEff = eff
      if (tracked > eff) {
        console.warn(`⚠️  ${f.name}: tracked=${tracked}/${eff} EXCEEDS MAX`)
        overloadedCount++
      }
    }
    if (overloadedCount === 0) {
      console.log(`✅ Load audit: All ${faculty.length} faculty within effective max (max: ${maxTracking}/${maxEff})${trimmedCount > 0 ? ` (trimmed ${trimmedCount} faculty)` : ''}`)
    } else {
      console.warn(`⚠️  Load audit: ${overloadedCount}/${faculty.length} faculty exceed effective max (max: ${maxTracking}/${maxEff})`)
    }
