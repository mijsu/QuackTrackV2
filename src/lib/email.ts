...
// Nodemailer is imported lazily to prevent any startup crash if the module
// has issues in the deployment environment. All SMTP settings are read lazily
// from environment variables inside each function call.
...
async function getNodemailer() {
  if (!nodemailerModule) {
    try {
      ...
      throw new Error('nodemailer module could not be loaded. Email features are unavailable.')
    }
  }
 ...
 * Called lazily so env vars are always read fresh.
 */
