...
export function GenerateView() {
  ...
  // Fetch departments for the multi-select
  const { data: departmentsData } = useQuery({
    queryKey: ['departments'],
    queryFn: async () => {
      const res = await api.get<{ data: DepartmentSummary[] }>('/departments')
      return res.data as { data: DepartmentSummary[] } | null
    },
  })
  const departments: DepartmentSummary[] = departmentsData?.data ?? []
  ...
  const generateMutation = useMutation({
    mutationFn: async (params: { semester: string; academicYear: string; departmentIds?: string[] }) => {
      const body: Record<string, unknown> = {
        semester: params.semester,
        academicYear: params.academicYear,
      }
      if (params.departmentIds && params.departmentIds.length > 0) {
        body.departmentIds = params.departmentIds
      }
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-muted-foreground">
                Departments <span className="text-foreground/30">(select one or more)</span>
              </label>
              <MultiSelect
                options={departments.map(d => ({
                  value: d.id,
                  label: `${d.name} (${d.code})`,
                  hint: `${d.classType} · ${d._count.subjects} subjects · ${d._count.users} faculty`,
                }))}
                    ...
                    {selectedDeptIds.map(id => {
                      ...
                      return (
                        <span key={id} className="inline-flex items-center gap-1">
                          <span className={`size-1.5 rounded-full ${d.classType === 'executive' ? 'bg-amber-400' : 'bg-[#10B981]'}`} />
                          {d.code}
                        </span>
                      )
                    })}
            ...
            <AlertDialogDescription asChild className="text-muted-foreground text-sm space-y-3 pt-1">
              <div>
                ...
                <div className="bg-secondary/50 border border-border rounded-xl p-4 space-y-2.5">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-muted-foreground text-xs">Departments</span>
                    <span className="text-xs font-semibold flex items-center gap-1.5">
                      {selectedDeptIds.length > 0 ? (
                        <span className="text-[#10B981]">{selectedDeptIds.length} selected</span>
                      ) : (
                  ...
                  {selectedDeptIds.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      {selectedDeptIds.map(id => {
                        ...
                        if (!d) return null
                        return (
                          <span key={id} className="text-[10px] font-mono px-2 py-0.5 rounded-md bg-secondary/50 border border-border">
                            {d.code}
                          </span>
                        )
                      })}
                ...
                {selectedDeptIds.length > 0 && (() => {
                  const hasExecutive = selectedDeptIds.some(id => {
                    const d = departments.find(dept => dept.id === id)
                    return d?.classType === 'executive'
                  })
                  const hasRegular = selectedDeptIds.some(id => {
                    const d = departments.find(dept => dept.id === id)
                    return d?.classType === 'regular'
                  })
                  ...
                  return (
                    <div className="flex items-start gap-2 p-3 bg-amber-500/5 border border-amber-500/20 rounded-lg">
                      ...
                      <span className="text-amber-400/90 text-xs">
                        {hasRegular && hasExecutive
                          ? 'Mixed department types selected — all faculty (regular and masteral) will be eligible.'
                          : 'Executive department selected — only masteral faculty (with master\'s degrees) will be assigned.'}
                      </span>
                    </div>
                  )
                })()}
