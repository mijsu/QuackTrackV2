    ...
    <div className="space-y-4">
      ...
      {merged.length === 0 ? (
        ...
        <div className="space-y-3">
          {merged.map(({ start, end, items }, idx) => {
            ...
            return (
              <div key={`${start}-${end}-${idx}`} className="border border-border rounded-lg overflow-hidden">
...
export function SchedulesView() {
  ...
  const { data: schedulesData, isLoading: schedulesLoading } = useQuery({
    ...
    queryFn: async () => { if (!currentVersionId) return null; const r = await api.get<{ data: ScheduleItem[]; pagination: { total: number } }>(`/schedules?scheduleVersionId=${currentVersionId}&limit=5000`); return r.data },
  ...
  const facultyStats = useMemo(() => {
    ...
    // Count merged classes (same logic as ScheduleGrid): group by
    // (day, subjectId, sectionId, facultyId) and merge consecutive slots.
    const byFaculty = new Map<string, { day: string; subjectId: string; sectionId: string; facultyId: string; startTime: string; endTime: string; units: number }[]>()
    classFilteredSchedules.forEach(sc => {
      const key = `${sc.day}|${sc.subjectId}|${sc.sectionId}|${sc.facultyId}`
      const arr = byFaculty.get(key) || []
      arr.push({ day: sc.day, subjectId: sc.subjectId, sectionId: sc.sectionId, facultyId: sc.facultyId, startTime: sc.startTime, endTime: sc.endTime, units: sc.subject?.units || 0 })
      byFaculty.set(key, arr)
    })
    // Merge consecutive slots, count as 1 class
    for (const [facKey, items] of byFaculty) {
      items.sort((a, b) => a.startTime.localeCompare(b.startTime))
      let mergedCount = 0
      let i = 0
      while (i < items.length) {
        mergedCount++ // Count each group of consecutive slots as 1 class
        let j = i + 1
        while (j < items.length && items[j].startTime === items[j-1].endTime) { j++ }
        i = j
      }
      const fid = items[0].facultyId
      if (!s[fid]) s[fid] = { count: 0, units: 0 }
      s[fid].count += mergedCount
      s[fid].units += items.reduce((sum, item) => sum + item.units, 0)
    }
    return s
  ...
  const sectionStats = useMemo(() => {
    ...
    // Count merged classes per section (same merge logic as facultyStats)
    const bySection = new Map<string, { day: string; subjectId: string; sectionId: string; startTime: string; endTime: string }[]>()
    classFilteredSchedules.forEach(sc => {
      const key = `${sc.day}|${sc.subjectId}|${sc.sectionId}|${sc.facultyId}`
      const arr = bySection.get(key) || []
      arr.push({ day: sc.day, subjectId: sc.subjectId, sectionId: sc.sectionId, startTime: sc.startTime, endTime: sc.endTime })
